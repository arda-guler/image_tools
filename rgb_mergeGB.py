...
def import_image():
    ...
    try:
        image1 = Image.open(filename + "_greenFiltered.png")
        image2 = Image.open(filename + "_blueFiltered.png")
        filename = filename + ".png"
    except:
        try:
            image1 = Image.open(filename + "_greenFiltered.jpg")
            image2 = Image.open(filename + "_blueFiltered.jpg")
...
def main():
    ...
    print("\nGB merging " + filetitle + fileextension + "\n")
    ...
    if channel_num == 3:
        for y in range(image1.size[1]):
            for x in range(image1.size[0]):
                i += 1
                r, g, b = 0, pixels1[x, y][1], pixels2[x, y][2]
                pixels1[x, y] = (r, g, b)
    ...
    # R, G, B, Alpha channels
        ...
        for y in range(image1.size[1]):
            for x in range(image1.size[0]):
                r, g, b, a = 0, pixels1[x, y][1], pixels2[x, y][2], 0 + pixels1[x, y][3] + pixels2[x, y][3]
                pixels1[x, y] = (r, g, b, a)
    ...
    image1.save(filetitle + "_gbFilterMerged" + fileextension)
